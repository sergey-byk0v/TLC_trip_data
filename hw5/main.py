...
def _main():
    columns, data = read_csv(sys.argv[1])
    users = [d[3] for d in data]
    prices = [float(d[2]) for d in data]
    stores = [d[1] for d in data]
    items = [d[0] for d in data]
    print(f'Unique value of stores: {len(unique_count(stores))}')
    print(f'Unique value of items: {len(unique_count(items))}')
    print(f'User with max price count: {max(unique_count(users), key=unique_count(users).get)}')
